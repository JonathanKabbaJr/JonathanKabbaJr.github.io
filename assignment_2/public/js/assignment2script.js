...
function getTaskName() {
  console.log("Called getTaskName");

  let taskNameParam = document.getElementById("taskNameSearch").value;
  console.log("taskNameParam:" + taskNameParam);
  let taskURL = "http://localhost:4000/taskNameSearch?taskName=" + taskNameParam;
  const fetchPromise = fetch(taskURL);
  ...
  fetchPromise
      ...
      .then((task) => {
        ...
        let taskId, taskName, taskDate;
        let message = "ERROR";
        if (typeof task.data.taskName !== "undefined") {
          taskName = task.data.taskName;
          taskId = task.data.id;
          taskFirstName = task.data.firstName;
          taskGroupName = task.data.groupName;
        ...
        document.getElementById("getTaskNameContent").innerHTML = message;
      })
      .catch((err) => {
        console.log(err);
        document.getElementById("getTaskNameContent").innerHTML = "Invalid task Name " + taskNameParam;
      });
...
function submitNewGroup() {
  ...
  let groupName = document.getElementById("addNewGroup").value;
...
async function getPageData(prepend = "") {
  await fetch('http://localhost:4000/allGroups', settings)
    .then(res => res.json())
    .then((json) => {
  ...
  await fetch('http://localhost:4000/allPeople', settings)
    .then(res => res.json())
    .then((json) => {
      let listSize = json.data.length;
      json.data.forEach(element => {
